...
const getPalette = (args: Args):false|Scheme => {
  ...
  if (!['any', 'warm', 'cold'].includes(query.temp)) {
    console.warn('temp option not recognized, the script will use default value (any)')
    query.temp = 'any'
  }
  if (!['any', 'bright', 'dark'].includes(query.theme)) {
    console.warn('theme option not recognized, the script will use default value (any)')
    query.theme = 'any'
  }
  if (query.rand < 0 || query.rand > 1) {
    console.warn('rand is not in the required range (0-1), the script will cast it.')
